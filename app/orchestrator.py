...
def _maybe_llm(
    ...
    system = (
        ...
        "Use only the supplied evidence. Never invent business facts, credentials, "
        "customer outcomes, revenue, prices, certifications, reviews, or legal claims. "
...
def run_pipeline(lead: dict[str, Any]) -> list[AgentResult]:
    """Run the deterministic 10-agent workflow sequentially."""
    adapter = LLMAdapter()
    ...
    results.append(
        _maybe_llm(
            ...
            "Evidence Auditor",
            audit,
            {"lead": lead, "qualification": results[-1].output},
        )
    )
    ...
    results.append(
        _maybe_llm(
            ...
            "Offer Strategist",
            offer,
            {"lead": lead, "audit": audit.output},
        )
    )

    # These stages are dependent, so calculate each output once and
    # pass it explicitly to the next stage.
