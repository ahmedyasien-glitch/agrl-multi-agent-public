...
st.set_page_config(
    page_title="AGRL Multi-Agent Research Business V2",
    page_icon="🧠",
    layout="wide",
)
...
st.caption(
    "Neon/PostgreSQL-backed, human-supervised workflow for GIS, research mapping, "
    "website audit, outreach and revenue tracking"
)
...
with st.sidebar:
    ...
    st.divider()
    st.warning(
        "Human approval is required before external messages, delivery, "
        "or recording a real sale."
    )
...
with pages[0]:
    ...
    with st.form("lead_form"):
        company = st.text_input(
            "Company / Researcher",
            placeholder="e.g. Environmental consultancy or research group",
        )
        ...
        problem_signal = st.text_area(
            "Observed problem / opportunity signal",
            placeholder="Describe one concrete issue observed from public information",
        )
        mapping_task = st.text_input(
            "Possible mapping / delivery task",
            value="One publication-ready map",
        )
        run = st.form_submit_button(
            "Save lead + run 10-agent pipeline",
            type="primary",
        )
    ...
    if run:
        lead = {
            ...
            "problem_signal": problem_signal.strip(),
            "mapping_task": mapping_task.strip(),
            "owner": "A.Y.",
        }
        ...
        proposal = next(
            r.output for r in results if r.agent == "Proposal Writer"
        )
        outreach = next(
            r.output for r in results if r.agent == "Outreach Agent"
        )
        ...
        st.success(
            f"Pipeline complete. Lead ID={lead_id}; Run={run_id}"
        )
...
with pages[1]:
    ...
    if not results:
        ...
        mgr = next(
            (r for r in results if r["agent"] == "Manager"),
            None,
        )
        ...
        if mgr:
            st.metric(
                "Manager decision",
                mgr["output"].get("decision", "UNKNOWN"),
            )
...
with pages[3]:
    ...
    if not leads:
        ...
        with cc2:
            ...
            dtype = st.text_input(
                "Deliverable type",
                value="Publication-ready GIS map",
            )
            fname = st.text_input(
                "File name",
                value="final_map.png",
...
st.divider()
st.caption(
    "AGRL GIS Mapping Support · Internal identity: A.Y. · "
    "No automatic email sending, no automatic money movement, no autonomous trading."
)
